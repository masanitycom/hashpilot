-- ========================================
-- V2関数のnft_sequenceエラー修正（NULL対応版）
-- ========================================
-- 問題: auto_nft_countがNULLの場合、NULL + 1 = NULL となり制約違反
-- 解決: COALESCEでNULLを0として扱う
-- ========================================
...
BEGIN
  ...
  FOR v_user_record IN
    SELECT
      u.user_id,
      COALESCE(SUM(CASE WHEN nm.buyback_date IS NULL THEN 1 ELSE 0 END), 0) as active_nft_count
    FROM users u
    ...
    LOOP
      FOR v_referral_record IN
        ...
        WHERE child.referrer_user_id = v_user_record.user_id
          ...
          AND grandchild.operation_start_date <= p_date
  ...
  IF v_distribution_dividend > 0 THEN
    ...
    LOOP
      FOR v_referral_record IN
        SELECT
          great_grandchild.user_id as child_user_id,
          COALESCE(SUM(ndp.daily_profit), 0) as child_profit
        FROM users child
        JOIN users grandchild ON grandchild.referrer_user_id = child.user_id
        JOIN users great_grandchild ON great_grandchild.referrer_user_id = grandchild.user_id
        ...
        WHERE child.referrer_user_id = v_user_record.user_id
          AND great_grandchild.has_approved_nft = true
          AND great_grandchild.operation_start_date IS NOT NULL
          AND great_grandchild.operation_start_date <= p_date
  ...
  FOR v_user_record IN
    SELECT
      ...
      u.id as user_uuid,
      ac.cum_usdt,
      COALESCE(ac.auto_nft_count, 0) as auto_nft_count  -- ★ NULL対応
    FROM users u
  ...
  LOOP
    INSERT INTO nft_master (
      ...
      v_user_record.user_id,
      'auto',
      v_user_record.auto_nft_count + 1,  -- auto_nft_countはCOALESCEで0保証済み
      p_date,
      NULL
    );
    ...
    SET
      ...
      auto_nft_count = COALESCE(auto_nft_count, 0) + 1,  -- ★ NULL対応
  ...
  RETURN jsonb_build_object(
    ...
    'details', jsonb_build_object(
      ...
      'personal_distribution', jsonb_build_object(
        'count', v_personal_count,
        'amount', v_total_personal
      ),
      'referral_distribution', jsonb_build_object(
        'count', v_referral_count,
        'amount', v_total_referral
      ),
...
-- 実行確認
-- ========================================
SELECT '✅ V2関数のnft_sequenceエラーを修正しました（NULL対応版）' as status;
