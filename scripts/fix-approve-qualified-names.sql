...
BEGIN
    ...
    UPDATE users
    SET
        total_purchases = users.total_purchases + v_amount_usd,
        updated_at = NOW()
    WHERE users.user_id = v_user_id;

    -- affiliate_cycleレコードの存在確認
    SELECT EXISTS(
...
BEGIN
    ...
    RAISE NOTICE '=========================================';
    RAISE NOTICE '修正内容:';
    RAISE NOTICE '  - 全UPDATE文でテーブル名を完全修飾';
    RAISE NOTICE '  - purchases.id, users.user_id, affiliate_cycle.user_id';
    RAISE NOTICE '=========================================';
END $$;
