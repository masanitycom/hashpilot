...
-- 本番環境用 V1関数修正
-- ========================================
-- 問題: purchasesにnft_quantityが抜けている可能性
-- 解決: nft_quantityとnft_sequenceを追加
-- ========================================
-- 実行環境: 本番環境 Supabase SQL Editor
-- ========================================

-- 注意: この修正は、テスト環境で成功確認後に本番環境で実行してください
...
BEGIN
    ...
    LOOP
      ...
      IF v_level1_user_id IS NOT NULL THEN
        IF EXISTS (SELECT 1 FROM users WHERE user_id = v_level1_user_id AND operation_start_date IS NOT NULL AND operation_start_date <= p_date) THEN
          ...
          UPDATE affiliate_cycle
          SET cum_usdt = cum_usdt + (v_user_profit * v_level1_rate), updated_at = NOW()
        ...
        IF v_level2_user_id IS NOT NULL THEN
          IF EXISTS (SELECT 1 FROM users WHERE user_id = v_level2_user_id AND operation_start_date IS NOT NULL AND operation_start_date <= p_date) THEN
            ...
            UPDATE affiliate_cycle
            SET cum_usdt = cum_usdt + (v_user_profit * v_level2_rate), updated_at = NOW()
          ...
          IF v_referral_user_id IS NOT NULL THEN
            IF EXISTS (SELECT 1 FROM users WHERE user_id = v_referral_user_id AND operation_start_date IS NOT NULL AND operation_start_date <= p_date) THEN
              ...
              UPDATE affiliate_cycle
              SET cum_usdt = cum_usdt + (v_user_profit * v_level3_rate), updated_at = NOW()
  ...
  -- ★ 修正: nft_sequence（最大値+1）とnft_quantityを追加
  FOR v_user_record IN
    SELECT user_id, cum_usdt, phase, COALESCE(auto_nft_count, 0) as auto_nft_count
    FROM affiliate_cycle
    WHERE cum_usdt >= 2200
      AND EXISTS (SELECT 1 FROM users WHERE users.user_id = affiliate_cycle.user_id AND operation_start_date IS NOT NULL AND operation_start_date <= p_date)
  LOOP
    -- ★ ユーザーの最大シーケンス番号を取得
    SELECT COALESCE(MAX(nft_sequence), 0) INTO v_max_sequence
    FROM nft_master WHERE user_id = v_user_record.user_id;
    ...
    -- ★ nft_quantity=1 と cycle_number_at_purchase を追加
    INSERT INTO purchases (user_id, nft_quantity, amount_usd, admin_approved, is_auto_purchase, cycle_number_at_purchase, created_at)
    VALUES (v_user_record.user_id, 1, 1100, TRUE, TRUE, v_user_record.auto_nft_count + 1, NOW());
  ...
  INSERT INTO daily_yield_log (date, yield_rate, margin_rate, user_rate, is_month_end, created_at)
  VALUES (p_date, p_yield_rate, p_margin_rate, v_user_rate, v_is_month_end, NOW());
...
SELECT '✅ 本番環境用V1関数を修正しました（nft_sequence, nft_quantity, cycle_number_at_purchase追加）' as status;
