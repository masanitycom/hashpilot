...
-- 3. 簡単な日利設定関数を作成（既存の関数が問題ある場合のバックアップ）
...
-- 5. daily_yield_logテーブルにdate列のUNIQUE制約がない場合は追加
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'daily_yield_log_date_key'
    ) THEN
...
-- 8. user_daily_profitの確認
SELECT 
    'User daily profit summary' as info,
    date,
    COUNT(*) as user_count,
