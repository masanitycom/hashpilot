-- ON CONFLICT句のambiguousエラーを修正
-- 問題: affiliate_cycle.manual_nft_count の参照でuser_idが曖昧
-- 修正: EXCLUDEDキーワードを使用
...
DECLARE
    ...
    v_current_manual_count INTEGER := 0;
    v_current_total_count INTEGER := 0;
BEGIN
    -- UUIDに変換
    BEGIN
    ...
    -- ★★★ 修正ポイント: affiliate_cycleの現在のカウントを先に取得 ★★★
    SELECT
        COALESCE(ac.manual_nft_count, 0),
        COALESCE(ac.total_nft_count, 0)
    INTO
        v_current_manual_count,
    ...
    ON CONFLICT (user_id) DO UPDATE SET
        manual_nft_count = v_current_manual_count + v_nft_quantity,
        total_nft_count = v_current_total_count + v_nft_quantity,
        last_updated = NOW();
...
BEGIN
    RAISE NOTICE '=========================================';
    RAISE NOTICE '✅ ON CONFLICT句のambiguousエラーを修正';
    RAISE NOTICE '=========================================';
    RAISE NOTICE '修正内容:';
    RAISE NOTICE '  - affiliate_cycleの現在値を先に変数に取得';
    RAISE NOTICE '  - ON CONFLICT句で変数を使用してuser_idの曖昧さを解消';
    RAISE NOTICE '=========================================';
END $$;
