-- ========================================
-- STEP 5: 誤配布データの削除（簡略版）
-- ========================================
-- ⚠️⚠️⚠️ この操作は取り消せません ⚠️⚠️⚠️
-- 実行前に必ずバックアップを確認してください
-- ========================================
--
-- 重要: affiliate_cycleの調整は不要です
-- 理由: STEP2の調査で、影響を受けた全ユーザーの available_usdt = 0 であることを確認済み
-- つまり、nft_daily_profitにレコードは存在するが、affiliate_cycleには金額が加算されていない
-- したがって、レコード削除のみで修正が完了します
-- ========================================
...
-- ========================================
-- STEP 5-2: 影響を受けるユーザーの確認
-- ========================================

-- ユーザーごとの削除対象データを確認（上位20件）
SELECT
    '👤 影響を受けるユーザー（上位20件）' as label,
    COALESCE(ndp_summary.user_id, urp_summary.user_id) as user_id,
    u.full_name,
    u.operation_start_date,
    COALESCE(ndp_summary.total_personal, 0) as personal_profit_to_delete,
    COALESCE(urp_summary.total_referral, 0) as referral_profit_to_delete,
    COALESCE(ndp_summary.record_count, 0) as ndp_records,
    COALESCE(urp_summary.record_count, 0) as urp_records,
