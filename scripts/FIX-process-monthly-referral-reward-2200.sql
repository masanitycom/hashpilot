...
DECLARE
  ...
  v_year_month TEXT;
  v_nft_to_grant INTEGER;  -- ★追加: 付与するNFT数
  v_i INTEGER;             -- ★追加: ループカウンタ
BEGIN
  -- ========================================
  -- STEP 1: 入力検証
  -- ========================================
  ...
  SET
    phase = CASE
      ...
      WHEN cum_usdt >= 2200 THEN 'NFT_PENDING'  -- NFT付与待ち
      ELSE 'HOLD'  -- $1100〜$2199
    END,
    updated_at = NOW()
  WHERE cum_usdt >= 0;
  ...
  -- ★修正: $2,200を減算（$1,100ではない）
  -- ★修正: WHILEループで複数NFT付与に対応
  -- ========================================
  FOR v_user_record IN
    SELECT
      ac.user_id,
  ...
  LOOP
    ...
    FOR v_i IN 1..v_nft_to_grant LOOP
      ...
      INSERT INTO purchases (
        ...
        true,
        NOW(),
        v_user_record.auto_nft_count + v_i,
        true
      );
...
SELECT 'process_monthly_referral_reward関数を修正しました（cum_usdt - 2200 + WHILEループ）' as result;
