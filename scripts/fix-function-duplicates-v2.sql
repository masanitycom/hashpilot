-- 日利処理関数の重複を解消するスクリプト（修正版）
-- 両方の関数を削除してから最新版のみを再作成
...
SELECT 
    ...
    CASE 
        ...
        WHEN p.pronargs = 5 THEN '🟢 最新版（削除後再作成）'
        ELSE '❓ 不明'
...
INSERT INTO system_logs (
    ...
    'SUCCESS',
    'function_complete_cleanup',
    NULL,
    'process_daily_yield_with_cycles関数の全バージョンを削除しました',
    jsonb_build_object(
        'deleted_functions', ARRAY[
            'process_daily_yield_with_cycles(DATE, NUMERIC, NUMERIC, BOOLEAN)',
            'process_daily_yield_with_cycles(DATE, NUMERIC, NUMERIC, BOOLEAN, BOOLEAN)'
        ],
        'reason', '関数重複エラーの完全解消'
    ),
    NOW()
...
INSERT INTO system_logs (
    ...
    'SUCCESS',
    'function_duplicate_resolved_v2',
    NULL,
    'process_daily_yield_with_cycles関数の重複問題を完全解決しました',
    jsonb_build_object(
        'action', '全バージョン削除後、最新5引数版を新規作成',
        'features', ARRAY['月末処理対応', '5%ボーナス', '15日運用開始', '改善されたエラーハンドリング', '月次統計記録'],
...
SELECT 
    ...
    auto_purchases_created,
    processing_time_seconds,
    test_mode,
    month_end_bonus_applied
