...
BEGIN
    ...
    -- ⭐ STEP 3: NFT自動付与処理
    FOR v_user_record IN
        SELECT user_id, cum_usdt, total_nft_count, auto_nft_count
...
BEGIN
    RAISE NOTICE '===========================================';
    RAISE NOTICE '✅ 日利処理に月末出金自動実行を追加しました';
    RAISE NOTICE '===========================================';
    RAISE NOTICE '動作:';
    RAISE NOTICE '  - 日利処理実行時に月末判定（日本時間）';
    RAISE NOTICE '  - 月末なら自動的に process_monthly_withdrawals() を実行';
    RAISE NOTICE '  - テストモードでは実行されない';
    RAISE NOTICE '';
    RAISE NOTICE '変更点:';
    RAISE NOTICE '  - 戻り値に monthly_withdrawals_processed 列を追加（8列）';
    RAISE NOTICE '  - 月末の場合、メッセージに出金処理件数を表示';
    RAISE NOTICE '===========================================';
END $$;
