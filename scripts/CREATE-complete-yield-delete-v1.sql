...
-- テスト環境で成功確認後に実行してください
-- ========================================

CREATE OR REPLACE FUNCTION delete_daily_yield_with_cycles(p_date DATE)
...
BEGIN
  ...
  -- 3. 紹介報酬を巻き戻す（affiliate_cycleのcum_usdtから減算）
  ...
  RETURN QUERY SELECT
    p_date || 'の日利データを完全に削除しました'::TEXT as message,
    jsonb_build_object(
      'date', p_date,
...
SELECT '✅ V1日利削除関数を作成しました（本番環境用・全テーブル完全ロールバック）' as status;
