...
-- ========================================
-- STEP 3: user_referral_profitから誤配布レコードを削除
-- ========================================
...
WHERE urp.date >= '2025-11-01'
  ...
  AND EXISTS (
    ...
    LEFT JOIN user_daily_profit udp ON urp2.child_user_id = udp.user_id AND urp2.date = udp.date
    WHERE urp2.user_id = urp.user_id
      AND urp2.child_user_id = urp.child_user_id
      AND urp2.date = urp.date
      AND urp2.referral_level = urp.referral_level
      AND COALESCE(udp.daily_profit, 0) = 0
      AND urp2.profit_amount > 0
  );
