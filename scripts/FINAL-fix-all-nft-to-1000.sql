...
-- STEP 1: 既存NFTを全て1000ドルに修正
UPDATE nft_master
SET nft_value = 1000.00
WHERE nft_value != 1000.00;

-- STEP 2: 日利処理関数を修正（自動NFT作成時に1000ドル）
...
BEGIN
    ...
    LOOP
        ...
        v_base_amount := v_user_record.total_nft_count * 1000;  -- ⭐ 1000ドル単位
        ...
        IF NOT p_is_test_mode THEN
            ...
            FOR i IN 1..v_auto_nft_count LOOP
                ...
                VALUES (
                    ...
                    'auto', 1000.00, p_date, NOW(), NOW()  -- ⭐ 1000ドルに修正
                );
            END LOOP;
...
SELECT
    '修正完了' as status,
    COUNT(*) FILTER (WHERE nft_value = 1000) as correct_1000,
    COUNT(*) FILTER (WHERE nft_value != 1000) as wrong_not_1000
FROM nft_master;
...
BEGIN
    RAISE NOTICE '===========================================';
    RAISE NOTICE '✅ 全NFTを1000ドルに統一しました';
    RAISE NOTICE '===========================================';
    RAISE NOTICE '修正内容:';
    RAISE NOTICE '  1. nft_master: 全NFTを1000ドルに変更';
    RAISE NOTICE '  2. 日利計算: NFT枚数 × 1000ドル × 日利率';
    RAISE NOTICE '  3. 自動NFT付与: 1000ドルで作成';
    RAISE NOTICE '';
    RAISE NOTICE '10/15からの日利設定で正しく動作します';
    RAISE NOTICE '===========================================';
END $$;
