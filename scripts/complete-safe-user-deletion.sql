-- 完全版：安全なユーザー削除関数（全テーブル対応）
-- 存在する全ての関連テーブルを確認してから削除

-- 1. 現在存在するテーブルを確認
SELECT 
    table_name,
    CASE 
        WHEN table_name LIKE '%user%' THEN '★ユーザー関連'
        WHEN table_name LIKE '%referral%' THEN '★紹介関連'
        WHEN table_name LIKE '%affiliate%' THEN '★アフィリエイト関連'
        ELSE 'その他'
    END as category
...
WHERE table_schema = 'public' 
    AND (
        ...
        OR table_name IN ('purchases', 'buyback_requests', 'system_logs', 'nft_holdings')
    )
ORDER BY category, table_name;

-- 2. 全ての外部キー制約を確認（users テーブルを参照しているもの）
SELECT 
    tc.table_name,
    tc.constraint_name,
    kcu.column_name,
...
-- 4. 権限設定
GRANT EXECUTE ON FUNCTION public.delete_user_safely(TEXT, TEXT, OUT TEXT, OUT TEXT, OUT JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION public.delete_user_safely(TEXT, TEXT, OUT TEXT, OUT TEXT, OUT JSONB) TO anon;
GRANT EXECUTE ON FUNCTION public.delete_user_safely(TEXT, TEXT, OUT TEXT, OUT TEXT, OUT JSONB) TO service_role;

-- 5. 削除可能なユーザーを安全に確認（withdrawal_requestsを除外）
SELECT 
    u.user_id,
    u.email,
    u.full_name,
...
-- 6. 動作確認
SELECT 'Complete user deletion function created with all table checks' as result,
       COUNT(DISTINCT table_name) as total_constrained_tables
FROM information_schema.table_constraints AS tc 
JOIN information_schema.key_column_usage AS kcu
  ON tc.constraint_name = kcu.constraint_name
JOIN information_schema.constraint_column_usage AS ccu
