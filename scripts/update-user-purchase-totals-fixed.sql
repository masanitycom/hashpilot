...
-- 特定ユーザーの詳細確認
SELECT 'User 2BF53B details:' as info;
SELECT 
    u.user_id,
    u.email,
    u.total_purchases as stored_total,
    COUNT(p.id) as total_purchases_count,
    COUNT(CASE WHEN p.admin_approved THEN 1 END) as approved_purchases_count,
    SUM(CASE WHEN p.admin_approved THEN p.amount_usd ELSE 0 END) as calculated_total
FROM users u
LEFT JOIN purchases p ON u.user_id = p.user_id
