-- 本番環境で安全にget_referral_tree関数を削除・再作成
-- 既存データに影響しない方法で実行
...
-- 3. 新しい関数を作成（target_user_idパラメータを使用）
...
-- 5. 関数の作成確認
SELECT 
    'get_referral_tree function recreated successfully' as status,
    proname as function_name,
    proargnames as parameters
FROM pg_proc 
WHERE proname = 'get_referral_tree';

-- 6. 簡単なテスト実行（既存ユーザーIDでテスト）
SELECT 
    'Function test' as test_type,
    COUNT(*) as record_count
FROM get_referral_tree('7241f7') 
LIMIT 1;