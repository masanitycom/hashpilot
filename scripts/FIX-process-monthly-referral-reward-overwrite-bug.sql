-- ========================================
-- process_monthly_referral_reward 上書きバグ修正
-- ========================================
-- 問題: p_overwrite=TRUEで再実行すると、cum_usdtが二重加算される
-- 原因: monthly_referral_profitを削除する際に、cum_usdtから減算していない
--
-- 修正: 削除前にcum_usdtから該当月の紹介報酬を減算する
-- ========================================
...
BEGIN
  ...
  IF EXISTS (
    ...
    ELSE
      -- ========================================
      -- ★★★ 修正: 削除前にcum_usdtから減算する ★★★
      -- ========================================
      RAISE NOTICE '上書きモード: 既存の紹介報酬をcum_usdtから減算します';

      FOR v_old_referral_record IN
      ...
      LOOP
        ...
        SET
          cum_usdt = cum_usdt - v_old_referral_record.total_profit,
          updated_at = NOW()
        WHERE user_id = v_old_referral_record.user_id;

        RAISE NOTICE 'ユーザー %: cum_usdtから $% を減算',
          v_old_referral_record.user_id,
          ROUND(v_old_referral_record.total_profit::NUMERIC, 2);
      END LOOP;

      -- 既存データを削除
      DELETE FROM monthly_referral_profit WHERE year_month = v_year_month;
      RAISE NOTICE '既存の紹介報酬レコードを削除しました';
    END IF;
  END IF;
...
-- 確認
-- ========================================
SELECT 'process_monthly_referral_reward関数を修正しました（上書き時のcum_usdt二重加算バグを修正）' as status;
