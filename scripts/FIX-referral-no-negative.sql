...
BEGIN
    ...
    -- STEP 1: 各NFTの日利を計算（個人利益 - マイナスも含む）
    FOR v_nft_record IN
    ...
    -- ⭐ STEP 3: 紹介報酬（マイナス日利時は0にする）⭐
    IF p_yield_rate > 0 THEN  -- プラス日利の時だけ紹介報酬を計算
        FOR v_user_record IN
            SELECT u.user_id, u.has_approved_nft, u.operation_start_date,
                   COALESCE(SUM(nm.nft_value), 0) as total_nft_value
            FROM users u
    ...
    RETURN QUERY SELECT
        ...
        format('処理完了: ユーザー数=%s, サイクル更新=%s, 自動NFT付与=%s, 紹介報酬=%s（マイナス日利時は0）, 月次出金=%s, 処理時間=%s',
            v_user_count, v_cycle_updates, v_auto_purchases, v_referral_count,
            v_monthly_withdrawal_count, v_processing_time)::text;
END;
$$;

SELECT '✅ 修正完了' as status;
SELECT '変更点: マイナス日利時は紹介報酬を0にする（IF p_yield_rate > 0）' as note;
