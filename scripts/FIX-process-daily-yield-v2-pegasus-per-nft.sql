-- ========================================
-- process_daily_yield_v2 修正
-- ペガサスをNFT単位でチェックするように変更
-- ========================================
...
BEGIN
  ...
  -- NFT数をカウント（ペガサスNFTを除外）
  -- 条件: ユーザーがペガサスでない、またはNFTがペガサスでない
  SELECT COUNT(*)
  ...
  WHERE nm.buyback_date IS NULL
    ...
    AND (nm.is_pegasus = FALSE OR nm.is_pegasus IS NULL)  -- NFT単位でペガサスチェック
    AND (u.is_pegasus_exchange = FALSE OR u.is_pegasus_exchange IS NULL OR
         EXISTS (SELECT 1 FROM nft_master nm2
                 WHERE nm2.user_id = u.user_id
                 AND nm2.id = nm.id
                 AND (nm2.is_pegasus = FALSE OR nm2.is_pegasus IS NULL)));
    ...
    LOOP
      ...
      FOR v_nft_record IN
        ...
        WHERE user_id = v_user_record.user_id
          ...
          AND (is_pegasus = FALSE OR is_pegasus IS NULL)  -- NFT単位でペガサスチェック
      LOOP
...
SELECT 'process_daily_yield_v2 修正完了（ペガサスをNFT単位でチェック）' as status;
