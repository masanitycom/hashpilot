-- NFT計算ベースを1100から1000に修正
-- 1NFT = 1100ドル購入、但し運用は1000ドル（100ドルは手数料）
...
BEGIN
    -- 利率計算（CLAUDE.mdに従った正しい計算）
    v_after_margin := p_yield_rate * (1 - p_margin_rate / 100);
    v_user_rate := v_after_margin * 0.6;  -- CLAUDE.mdの仕様通り
    ...
    LOOP
        ...
        -- 修正: 基準金額（NFT数 × 1000）運用額ベース
        v_base_amount := v_nft_count * 1000;
        
        -- 個人投資分の利益計算
        v_personal_profit := v_base_amount * v_user_rate;
        
        -- まず個人利益のみでuser_daily_profitに保存
        IF NOT p_is_test_mode THEN
    ...
    LOOP
        ...
        -- 会社利益計算
        v_company_profit := COALESCE(v_personal_profit, 0) * p_margin_rate / 100;
    ...
    RETURN QUERY SELECT 
        ...
        CASE 
            WHEN p_is_test_mode THEN 
                format('テスト完了: %s名処理予定、総額$%s配布予定（1000ドル/NFTベース）', 
                       v_user_count, v_total_user_profit::text)
            ELSE 
                format('処理完了: %s名に総額$%s配布（1000ドル/NFTベース）', 
                       v_user_count, v_total_user_profit::text)
        END;
END;
$$;
...
-- CLAUDE.mdを更新
UPDATE CLAUDE_md_memo SET note = '
修正完了: NFT計算ベースを1000ドルに修正
- 1NFT = 1100ドル購入、運用は1000ドル
- 2000ドルユーザー: 2NFT × 1000 × 利率
- 1000ドルユーザー: 1NFT × 1000 × 利率
- ユーザー受取率 = マージン後利率 × 0.6 (CLAUDE.md仕様通り)
' WHERE id = 1;

SELECT 'NFT計算ベースを1000ドルに修正しました。CLAUDE.md仕様に準拠します。' as message;