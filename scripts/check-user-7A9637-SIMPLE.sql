-- ========================================
-- ユーザー7A9637のダッシュボードデータ確認（簡易版）
-- ========================================

-- 3. affiliate_cycle（サイクル情報）- cycle_countを削除
SELECT
    '3. サイクル情報' as section,
    total_nft_count,
    manual_nft_count,
...
SELECT
    ...
    SUM(ndp.daily_profit) as total_from_nft_table
FROM yesterday y
LEFT JOIN nft_daily_profit ndp ON ndp.user_id = '7A9637' AND ndp.date = y.yesterday_date
GROUP BY y.yesterday_date;
...
-- 10. ダッシュボードカードの期待値（重要！）
WITH yesterday AS (
    SELECT CURRENT_DATE - INTERVAL '1 day' as yesterday_date
)
SELECT
    '10. DailyProfitCard期待値' as section,
    y.yesterday_date,
    udp.daily_profit as personal_profit,
    udp.user_rate as user_rate_value,
    CASE
        WHEN udp.user_rate IS NOT NULL THEN CONCAT((udp.user_rate * 100)::TEXT, '%')
        ELSE 'NULL ← これが原因！'
    END as user_rate_display,
    CASE
        WHEN udp.user_rate IS NOT NULL AND udp.user_rate != 0
        THEN '✅ DailyProfitCardに表示される'
        WHEN udp.user_rate IS NULL
        THEN '❌ user_rateがNULL → $0.000表示'
        ELSE '❌ user_rateが0 → $0.000表示'
    END as status
FROM yesterday y
LEFT JOIN user_daily_profit udp ON udp.user_id = '7A9637' AND udp.date = y.yesterday_date;

-- 11. PersonalProfitCard期待値
WITH yesterday AS (
    SELECT CURRENT_DATE - INTERVAL '1 day' as yesterday_date
)
SELECT
    '11. PersonalProfitCard期待値' as section,
    y.yesterday_date,
    udp.daily_profit as personal_profit,
    CASE
        WHEN udp.daily_profit IS NOT NULL THEN '✅ PersonalProfitCardに表示される'
        ELSE '❌ データなし'
    END as status
FROM yesterday y
LEFT JOIN user_daily_profit udp ON udp.user_id = '7A9637' AND udp.date = y.yesterday_date;

-- 12. TotalProfitCard期待値
WITH yesterday AS (
    SELECT CURRENT_DATE - INTERVAL '1 day' as yesterday_date
)
SELECT
    '12. TotalProfitCard期待値' as section,
    y.yesterday_date,
    COALESCE(udp.daily_profit, 0) as personal_profit,
    COALESCE(urp.profit_amount, 0) as referral_profit,
    COALESCE(udp.daily_profit, 0) + COALESCE(urp.profit_amount, 0) as total_profit,
    CASE
        WHEN (COALESCE(udp.daily_profit, 0) + COALESCE(urp.profit_amount, 0)) > 0
        THEN '✅ TotalProfitCardに表示される'
        ELSE '⚠️ 合計が0 → $0.000表示'
    END as status
FROM yesterday y
LEFT JOIN user_daily_profit udp ON udp.user_id = '7A9637' AND udp.date = y.yesterday_date
