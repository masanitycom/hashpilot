-- ========================================
-- approve_user_nft関数を強制的に再作成
-- ========================================

-- 全てのバリエーションを削除
DROP FUNCTION IF EXISTS approve_user_nft(TEXT, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS approve_user_nft(TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS approve_user_nft(UUID, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS approve_user_nft(UUID, TEXT) CASCADE;
...
BEGIN
    ...
    ON CONFLICT (user_id) DO UPDATE SET
        manual_nft_count = affiliate_cycle.manual_nft_count + EXCLUDED.manual_nft_count,
        total_nft_count = affiliate_cycle.total_nft_count + EXCLUDED.total_nft_count,
        last_updated = NOW();
...
-- テスト
SELECT '✅ approve_user_nft関数を完全に再作成しました' as status;
SELECT '✅ v_target_user_id変数を使用してuser_idの曖昧さを完全に解消' as note;
