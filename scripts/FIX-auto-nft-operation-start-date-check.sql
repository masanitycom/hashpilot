-- ========================================
-- process_daily_yield_v2 関数の修正
-- NFT自動付与時にoperation_start_dateをチェック
-- ========================================
--
-- 問題: STEP 4（NFT自動付与）で、operation_start_dateのチェックがなかった
-- 結果: 運用開始前のユーザーにも自動NFTが付与されていた
--
-- 修正日: 2025-11-19
-- ========================================
...
BEGIN
  ...
  -- 1 NFTあたりの配当額
  v_user_profit_per_nft := v_distribution_dividend / v_total_nft_count;
  ...
  IF v_distribution_dividend != 0 THEN
    ...
    LOOP
      v_user_profit := v_user_profit_per_nft * v_user_record.nft_count;
  ...
  IF v_distribution_dividend > 0 THEN
    ...
    LOOP
      -- Level 1: 直接紹介者（ペガサス交換ユーザーも含む）
      FOR v_referral_record IN
        SELECT
          child.user_id as child_user_id,
          COUNT(nm.id) as child_nft_count
        FROM users child
        JOIN nft_master nm ON child.user_id = nm.user_id AND nm.buyback_date IS NULL
      ...
      LOOP
        -- 子ユーザーの仮の日利を計算
        v_user_profit := v_user_profit_per_nft * v_referral_record.child_nft_count;

        IF v_user_profit > 0 THEN
  ...
  -- ✅ 修正: operation_start_dateチェックを追加
  -- ========================================
  FOR v_user_record IN
    SELECT
      u.user_id,
    ...
    WHERE ac.cum_usdt >= 2200
      AND u.operation_start_date IS NOT NULL      -- ✅ 追加
      AND u.operation_start_date <= p_date        -- ✅ 追加
  LOOP
...
-- ========================================
-- この関数を本番環境に適用する前に、
-- INVESTIGATE-1109-auto-nft-bug.sql で問題を確認してください
-- ========================================
