...
-- ペガサスユーザーも運用開始日が来たら日利対象に含める
-- 実行日: 2026-01-20
-- ========================================

-- 変更点:
-- 削除: AND (u.is_pegasus_exchange = FALSE OR u.is_pegasus_exchange IS NULL)
-- → 運用開始日が来ていれば、ペガサスユーザーも日利対象に含める

-- 関連テーブルのカラム確認済み:
-- nft_daily_profit: nft_id, user_id, date, daily_profit, yield_rate, user_rate, base_amount, phase, created_at
-- stock_fund: user_id, date, stock_amount, cumulative_stock, source, notes, created_at
-- affiliate_cycle: user_id, cum_usdt, available_usdt, phase, etc.
...
SELECT 'process_daily_yield_v2 修正完了（ペガサスユーザー含む）' as status;
