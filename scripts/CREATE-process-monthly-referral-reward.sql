...
-- 機能:
-- 1. 指定月の各ユーザーの日利合計を計算
-- 2. 紹介ツリーを辿ってLevel 1/2/3の報酬を計算
-- 3. user_referral_profit_monthlyテーブルに保存
-- 4. affiliate_cycleを更新
--
-- 使い方:
-- SELECT * FROM process_monthly_referral_reward(2025, 11);
--
-- 作成日: 2025-12-01
-- ========================================
...
-- 月次紹介報酬テーブル作成
-- ========================================
CREATE TABLE IF NOT EXISTS user_referral_profit_monthly (
  ...
  child_user_id VARCHAR(10) NOT NULL REFERENCES users(user_id),
  child_monthly_profit NUMERIC NOT NULL,
  profit_amount NUMERIC NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(user_id, year, month, referral_level, child_user_id)
);

CREATE INDEX IF NOT EXISTS idx_user_referral_profit_monthly_user
  ON user_referral_profit_monthly(user_id);
CREATE INDEX IF NOT EXISTS idx_user_referral_profit_monthly_year_month
  ON user_referral_profit_monthly(year, month);
...
BEGIN
  RAISE NOTICE '=========================================';
  RAISE NOTICE '✅ 月末紹介報酬計算関数作成完了';
  RAISE NOTICE '=========================================';
  RAISE NOTICE 'テーブル: user_referral_profit_monthly';
  RAISE NOTICE '関数: process_monthly_referral_reward(year, month, overwrite)';
  RAISE NOTICE '';
  RAISE NOTICE '使い方:';
