...
-- ON CONFLICTを使わず、先にレコード存在確認してからINSERTまたはUPDATE
...
BEGIN
    ...
    -- ★★★ 修正: affiliate_cycleレコードの存在確認 ★★★
    SELECT EXISTS(
        SELECT 1 FROM affiliate_cycle ac WHERE ac.user_id = v_user_id
    ) INTO v_cycle_exists;
    ...
    IF v_cycle_exists THEN
        ...
        SET
            manual_nft_count = manual_nft_count + v_nft_quantity,
            total_nft_count = total_nft_count + v_nft_quantity,
            last_updated = NOW()
        WHERE affiliate_cycle.user_id = v_user_id;
    ELSE
        -- 新規レコードをINSERT
...
BEGIN
    RAISE NOTICE '=========================================';
    RAISE NOTICE '✅ approve_user_nft関数を完全修正';
    RAISE NOTICE '=========================================';
    RAISE NOTICE '修正内容:';
    RAISE NOTICE '  - ON CONFLICTを使用せず、IF文で分岐';
