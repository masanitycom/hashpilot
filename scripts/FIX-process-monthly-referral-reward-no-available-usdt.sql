-- ========================================
-- process_monthly_referral_reward関数修正
-- ========================================
-- 修正日: 2026-02-06
--
-- 変更点:
-- 1. 紹介報酬をavailable_usdtに加算しない（cum_usdtのみ）
-- 2. NFT自動付与時もavailable_usdtを変更しない
-- 3. phaseは従来通りcum_usdtで判定
-- ========================================
...
BEGIN
  ...
  -- ========================================
  -- STEP 3: Level 1 紹介報酬 (20%)
  -- ★ 修正: available_usdtは更新しない
  -- ========================================
  ...
  LOOP
    ...
    LOOP
      ...
      -- ★ cum_usdtのみ更新、available_usdtは触らない
      UPDATE affiliate_cycle
      SET
        cum_usdt = cum_usdt + (v_child_record.child_monthly_profit * v_level1_rate),
        phase = CASE
          WHEN (cum_usdt + (v_child_record.child_monthly_profit * v_level1_rate)) >= 1100 THEN 'HOLD'
          ELSE 'USDT'
        END,
        updated_at = NOW()
  ...
  LOOP
    ...
    LOOP
      ...
      SET
        ...
        phase = CASE
          WHEN (cum_usdt + (v_child_record.child_monthly_profit * v_level2_rate)) >= 1100 THEN 'HOLD'
          ELSE 'USDT'
        END,
        updated_at = NOW()
      WHERE user_id = v_user_record.user_id;
  ...
  LOOP
    ...
    LOOP
      ...
      SET
        ...
        phase = CASE
          WHEN (cum_usdt + (v_child_record.child_monthly_profit * v_level3_rate)) >= 1100 THEN 'HOLD'
          ELSE 'USDT'
        END,
        updated_at = NOW()
      WHERE user_id = v_user_record.user_id;
  ...
  -- ========================================
  -- STEP 6: NFT自動購入（cum_usdt >= 2200）
  -- ★ cum_usdt -= $2,200
  -- ★ available_usdtは変更しない
  -- ========================================
  FOR v_user_record IN
    SELECT
      ac.user_id,
  ...
  LOOP
    ...
    -- ★ affiliate_cycle更新（available_usdtは触らない）
    UPDATE affiliate_cycle
    SET
  ...
  RETURN QUERY SELECT
    'SUCCESS'::TEXT,
    format('%s年%s月の紹介報酬計算完了: %s名に総額$%s配布、NFT自動購入: %s件',
...
SELECT '✅ process_monthly_referral_reward関数を修正しました' as status;
SELECT '  - 紹介報酬はcum_usdtのみ更新（available_usdtは触らない）' as fix1;
SELECT '  - NFT自動購入時もavailable_usdtは変更しない' as fix2;
SELECT '  - cum_usdt >= $2200 で NFT購入、$2200減算' as fix3;
