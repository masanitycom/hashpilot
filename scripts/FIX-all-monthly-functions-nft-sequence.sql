-- ========================================
-- 全ての月末処理関数のNFT自動付与を修正
-- nft_sequenceとnft_valueを正しく設定
-- 実行日: 2026-01-13
-- ========================================
...
-- 1-2: 関連するpurchasesレコードを削除（is_auto_purchaseで判定）
...
-- ========================================
-- STEP 2: process_monthly_referral_profit関数を修正
-- ========================================
...
BEGIN
    ...
    LOOP
        ...
        -- NFT作成（nft_sequenceとnft_valueを正しく設定）
    ...
    RETURN QUERY SELECT 'SUCCESS'::TEXT, format('月次紹介報酬計算が完了しました（%s）', p_year_month)::TEXT,
...
BEGIN
  ...
  RAISE NOTICE '2. process_monthly_referral_profit関数のnft_sequence/nft_value追加';
END $$;
