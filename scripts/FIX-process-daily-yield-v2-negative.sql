-- ========================================
-- RPC関数: process_daily_yield_v2 修正版
-- マイナス日利でもユーザーに配布する
-- ========================================
--
-- 修正内容:
-- 1. Step 9: マイナス日利でも配当を計算（0にしない）
-- 2. Step 11-13: 条件を「!= 0」に変更（マイナスでも処理）
--
-- 修正日: 2025-11-13
-- ========================================
...
BEGIN
  ...
  -- ========================================
  -- Step 9: 分配計算（⭐修正: マイナスでも計算）
  -- ========================================
  -- ⭐ 修正前: IF v_daily_pnl > 0 THEN ... ELSE 0
  -- ⭐ 修正後: プラスでもマイナスでも配分を計算
  ...
  -- ========================================
  -- Step 11: 各ユーザーに配当を配分（⭐修正: != 0）
  -- ========================================
  -- ⭐ 修正前: IF v_distribution_dividend > 0 THEN
  -- ⭐ 修正後: IF v_distribution_dividend != 0 THEN
  ...
  -- ========================================
  -- Step 12: アフィリエイト報酬の配分（⭐修正: != 0）
  -- ========================================
  -- ⭐ 修正前: IF v_distribution_affiliate > 0 THEN
  -- ⭐ 修正後: IF v_distribution_affiliate != 0 THEN
  ...
  -- ========================================
  -- Step 13: ストック資金の記録（⭐修正: != 0）
  -- ========================================
  -- ⭐ 修正前: IF v_distribution_stock > 0 THEN
  -- ⭐ 修正後: IF v_distribution_stock != 0 THEN
  ...
  RETURN QUERY SELECT
    'SUCCESS'::TEXT,
    format('日利計算完了（マイナスも配布）: %s', p_date)::TEXT,
    jsonb_build_object(
      'date', p_date,
...
新しい累積ベースの日利計算（金額入力方式）- マイナス日利対応版

⭐ 修正内容（2025-11-13）:
  - マイナス日利でもユーザーに配布するように変更
  - Step 9: マイナスでも配当計算（0にしない）
  - Step 11-13: 条件を「!= 0」に変更

入力:
  p_date: 日付
  p_total_profit_amount: 全体運用利益（全NFT合計の金額、マイナス可）
...
処理フロー:
  ...
  4. 分配計算（配当60%, アフィリ30%, ストック10%）※マイナスでも計算
  5. 各ユーザーに配分（マイナスでも配布）
  6. 整合性チェック
...
DO $$
BEGIN
  RAISE NOTICE '✅ RPC関数 process_daily_yield_v2 をマイナス日利対応版に更新しました';
  RAISE NOTICE '   - マイナス日利でもユーザーに配布されます';
END $$;
