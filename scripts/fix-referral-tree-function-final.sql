-- 紹介ツリー関数の最終修正（levelカラム名の問題を解決）

-- 既存の関数を削除
DROP FUNCTION IF EXISTS get_referral_tree(VARCHAR(6));
DROP FUNCTION IF EXISTS get_referral_stats(VARCHAR(6));
...
BEGIN
    ...
    WITH RECURSIVE referral_tree AS (
        ...
        SELECT 
            1 as tree_level,
            u.user_id::VARCHAR(6) as tree_user_id,
            u.email::VARCHAR(255) as tree_email,
            COALESCE(
            ...
            u.referrer_user_id::VARCHAR(6) as tree_referrer_id
        FROM users u
        ...
        SELECT 
            ...
            u.referrer_user_id::VARCHAR(6)
        FROM users u
        INNER JOIN referral_tree rt ON u.referrer_user_id = rt.tree_user_id
        WHERE rt.tree_level < 3  -- 最大3レベルまで
    )
    SELECT 
        rt.tree_level as level_num,
        rt.tree_user_id as user_id,
        rt.tree_email as email,
        rt.tree_purchases as personal_purchases,
        -- 各ユーザーの下位ツリーの合計を計算
        COALESCE(
            (SELECT SUM(sub.tree_purchases) 
             FROM referral_tree sub 
             WHERE sub.tree_referrer_id = rt.tree_user_id), 
            0::NUMERIC
        ) as subtree_total,
        rt.tree_referrer_id as referrer_id,
        -- 直接紹介した人数
        (SELECT COUNT(*)::INTEGER 
         FROM users direct 
         WHERE direct.referrer_user_id = rt.tree_user_id) as direct_referrals_count
    FROM referral_tree rt
    ORDER BY rt.tree_level, rt.tree_user_id;
END;
$$;

-- 修正版：紹介ツリーの統計情報を取得する関数
...
-- テスト実行
SELECT 
    'Fixed Referral Tree Test' as check_type,
    *
FROM get_referral_tree('2BF53B')
LIMIT 5;

SELECT 
    'Fixed Referral Stats Test' as check_type,
    *
