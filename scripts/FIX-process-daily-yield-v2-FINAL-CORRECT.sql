-- ========================================
-- RPC関数: process_daily_yield_v2 最終修正版
-- 4つの重大な問題を修正
-- ========================================
--
-- 修正内容:
-- 1. 紹介報酬の計算: nft_masterから直接計算（ペガサス交換ユーザーも含む）
-- 2. NFT自動付与: cum_usdt -= 2200（1100ではない）
-- 3. affiliate_cycle更新: 紹介報酬はcum_usdtのみに加算
-- 4. NFT自動付与: プラス日利の時のみ実行（マイナス日利ではスキップ）
--
-- 修正日: 2025-11-23
-- ========================================
    ...
    LOOP
      ...
      LOOP
        ...
        IF v_user_profit > 0 THEN
          ...
          -- ❌ 修正: 紹介報酬はcum_usdtのみに加算（available_usdtには加算しない）
          UPDATE affiliate_cycle
          SET
