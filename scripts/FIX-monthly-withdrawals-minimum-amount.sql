-- ========================================
-- 🚨 緊急修正: 月末出金処理の最低出金額を100→10に修正
-- ========================================
-- 問題: process_monthly_withdrawals関数が available_usdt >= 100 でチェックしているため
--       $10～$99のユーザーが出金対象から除外されている
-- 影響: 2025年11月の処理で58名中56名が除外された（2名のみ処理）
-- 修正: available_usdt >= 10 に変更
-- ========================================

CREATE OR REPLACE FUNCTION public.process_monthly_withdrawals(
...
BEGIN
    ...
    FOR v_user_record IN
        ...
        WHERE ac.available_usdt >= 10  -- ✅ 修正: 100 → 10
          -- ⭐ ペガサス交換ユーザーで出金制限期間内のユーザーを除外
          AND NOT (
...
BEGIN
    RAISE NOTICE '===========================================';
    RAISE NOTICE '✅ process_monthly_withdrawals 修正完了';
    RAISE NOTICE '===========================================';
    RAISE NOTICE '変更内容:';
    RAISE NOTICE '  - 最低出金額: 100 USDT → 10 USDT';
    RAISE NOTICE '';
    RAISE NOTICE '次のステップ:';
    RAISE NOTICE '  1. 既存の2件のレコードを削除';
    RAISE NOTICE '  2. 11月分を再処理: SELECT * FROM process_monthly_withdrawals(''2025-11-01''::DATE);';
    RAISE NOTICE '  3. 結果確認: SELECT COUNT(*), SUM(total_amount) FROM monthly_withdrawals WHERE withdrawal_month = ''2025-11-01'';';
    RAISE NOTICE '===========================================';
END $$;
