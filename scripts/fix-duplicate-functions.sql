-- 本番環境の重複関数問題を修正するSQL
-- "Could not choose the best candidate function" エラーを解決

-- 1. 現在の関数の状況を確認
SELECT 
    'Current function overloads' as info,
    proname as function_name,
    pronargs as param_count,
    proargtypes::regtype[] as param_types
FROM pg_proc 
WHERE proname IN ('get_referral_stats', 'get_referral_tree')
ORDER BY proname, pronargs;

-- 2. get_referral_statsの全てのオーバーロードを削除
DROP FUNCTION IF EXISTS get_referral_stats(TEXT);
DROP FUNCTION IF EXISTS get_referral_stats(VARCHAR);
DROP FUNCTION IF EXISTS get_referral_stats(character varying);

-- 3. get_referral_treeの全てのオーバーロードを削除
...
-- 4. get_referral_tree関数を再作成（TEXTパラメータのみ）
...
-- 5. get_referral_stats関数を再作成（TEXTパラメータのみ）
...
-- 8. 動作テスト
SELECT 
    'Tree function test' as test_type,
    user_id,
    email,
    level_num,
    personal_investment
...
SELECT 
    'Stats function test' as test_type,
