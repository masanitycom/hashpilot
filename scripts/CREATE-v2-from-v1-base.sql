...
BEGIN
      ...
      IF v_level1_user_id IS NOT NULL THEN
        IF EXISTS (
          ...
          INSERT INTO user_referral_profit (
            user_id, date, referral_level, child_user_id, profit_amount, created_at
          ) VALUES (
            v_level1_user_id, p_date, 1, v_user_record.user_id, v_user_profit * v_level1_rate, NOW()
          );
        ...
        IF v_level2_user_id IS NOT NULL THEN
          IF EXISTS (
            ...
            INSERT INTO user_referral_profit (
              ...
              v_level2_user_id, p_date, 2, v_user_record.user_id, v_user_profit * v_level2_rate, NOW()
            );
          ...
          IF v_referral_user_id IS NOT NULL THEN
            IF EXISTS (
              ...
              INSERT INTO user_referral_profit (
                ...
                v_referral_user_id, p_date, 3, v_user_record.user_id, v_user_profit * v_level3_rate, NOW()
              );
  ...
  -- NFT自動付与（修正版: nft_sequence, nft_value追加）
  FOR v_user_record IN
    SELECT
      ac.user_id,
      ac.cum_usdt,
    ...
    WHERE ac.cum_usdt >= 2200
      AND EXISTS (
        SELECT 1 FROM users
        WHERE users.user_id = ac.user_id
          AND operation_start_date IS NOT NULL
          AND operation_start_date <= p_date
      )
  LOOP
  ...
  INSERT INTO daily_yield_log_v2 (
    ...
    v_total_nft_count,
    v_personal_profit_per_nft,
    v_cumulative_gross_profit,
    0.30,
    v_cumulative_fee,
    v_cumulative_net_profit,
    v_daily_pnl,
    v_distribution_dividend,
    p_total_profit_amount * 0.30,
    p_total_profit_amount * 0.10,
...
SELECT '✅ V2関数を作成しました（本番V1をベースに金額入力版に変換）' as status;
