...
BEGIN
  ...
  IF v_distribution_affiliate > 0 THEN
    FOR v_referral_record IN
      ...
      GROUP BY u.user_id, u.referrer_user_id
    LOOP
      v_user_nft_count := v_referral_record.nft_count;
      v_user_profit := (v_distribution_dividend / v_total_nft_count) * v_user_nft_count;
      ...
      IF EXISTS (
        ...
        INSERT INTO user_referral_profit (
          ...
          v_referral_record.referrer_user_id, p_date, 1,
          v_referral_record.user_id, v_user_profit * v_level1_rate, NOW()
        );
  ...
  LOOP
    INSERT INTO nft_master (user_id, nft_type, acquired_date, created_at)
    VALUES (v_user_record.user_id, 'auto', p_date, NOW());
  ...
  INSERT INTO daily_yield_log_v2 (
    ...
    distribution_stock, is_month_end, created_at
  ) VALUES (
    ...
    v_distribution_stock, v_is_month_end, NOW()
  );
...
COMMENT ON FUNCTION process_daily_yield_v2 IS 'V2日利処理（金額入力、月末処理統合版）';
