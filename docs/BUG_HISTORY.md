...
**問題:**
`/admin/withdrawals` のCSVで「月初NFT / 月末NFT / 手動NFT」の枚数が実際より少なく出ていた。

- A512FF: 実際8枚 → CSVでは**6枚**
- **26名が0枚**と表示（E28F37, 076D31, 8C3B2B ほか）
- CSV全体の月末NFT合計が**ちょうど1000**

**原因: Supabaseの1000件制限**

`app/admin/withdrawals/page.tsx` のNFT取得クエリに件数指定がなく、
Supabaseのデフォルト上限1000件で打ち切られていた。
...
const { data: nftData } = await supabase
  ...
  .in("user_id", userIds)
  .is("buyback_date", null)     // ← 1000件で打ち切り
```

434ユーザー分のNFTは1000枚を超える（91枚保有のユーザー等がいる）。
1000枚目以降が丸ごと捨てられ、境界にあたったユーザーは一部だけ、
それ以降のユーザーは0枚として集計されていた。

**修正内容:**
1000件ずつのページ分割で全件取得するよう変更。
`user_id` / `nft_sequence` 順に並べ、境界での取りこぼしをなくした。

**影響範囲:**
表示のみ。日利配布・出金金額の計算には影響しない
（日利は `nft_master` をDB側で直接集計しているため）。
ただしCSVは送金作業に使われるため、枚数の誤りは運用判断を誤らせる。

**同種の既知問題:**
`docs/BUSINESS_LOGIC.md` の「🚨 Supabaseの1000件制限に注意」に、
`monthly_referral_profit` で同じ問題が起きた経緯が記録されている。
そちらは月ごとの分割取得で回避済みだが、**このNFT取得は対策漏れだった**。
`.in()` で多数のユーザーを指定するクエリは全て要注意。
